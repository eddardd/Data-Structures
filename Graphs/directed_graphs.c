...
Graph* tranposeGraph(Graph* G);             // Transpose Graph
    ...
    for(u=0;u<G->n;u++){ // runs adjacency list of u
        ...
        while(aux1!=NULL){ // runs adjacency list of v
            ...
            while(aux2!=NULL){ // runs adjacency list of w
                ...
                if(phi[u][w] == 0){ // see if the edge wasn't added before
                    phi[u][w] = 1;
                    addEdge(H,u,w);
                }
            }
...
Graph* tranposeGraph(Graph* G){
    int u,v;
    node *aux;
    Graph* H = createGraph(G->n);
    for(u=0;u<G->n;u++){
        aux = G->array[u].head; // Adjacency list head
        while(aux!=NULL){ // runs adjacency list of v
            v = aux->dest; // v is adjacent to u
            aux = aux->nxt; // follows the adjacency list
            addEdge(H,v,u);
        }
    }
    return H;
